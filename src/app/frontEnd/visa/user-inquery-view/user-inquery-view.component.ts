...
@Component({
  selector: 'app-user-inquery-view',
  templateUrl: './user-inquery-view.component.html',
  styleUrls: ['./user-inquery-view.component.scss']
})


export class UserInqueryViewComponent implements OnInit{
...
this.service.searchVisa(data.value.holderPassportNo, data.value.holderDateOfBirth, data.value.holderNationality).subscribe({
  next: r=>{
    ...
    if(this.searchedData.length <1){
     ...
     this.router.navigate(["/visaRelatedInformationAndTracking/message//" + this.searchedData[0].id])  
    }
